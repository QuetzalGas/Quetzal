class TreeNode:
    ...
    def insertItem(self, newItem):
        ...
        if self.amount == 0:
            ...
            if newItem.key < self.items[0].key:
            ...
            if newItem.key < self.items[0].key:
                ...
                self.items[1] = self.items[0]
                self.items[0] = newItem
            elif newItem.key > self.items[1].key:
    ...
    def insertChild(self, newChild):
        ...
        if self.children[0] is None:
            ...
            if newChild.getMaxkey() < self.children[0].getMaxkey():
            ...
            if newChild.getMaxkey() < self.children[0].getMaxkey():
                ...
                self.children[1] = self.children[0]
                self.children[0] = newChild
            elif newChild.getMaxkey() < self.children[1].getMaxkey():
                self.children[2] = self.children[1]
                self.children[1] = newChild
            else:
            ...
            if newChild.getMaxkey() < self.children[0].getMaxkey():
                ...
                self.children[2] = self.children[1]
                self.children[1] = newChild
            elif newChild.getMaxkey() < self.children[2].getMaxkey():
                self.children[3] = self.children[2]
                self.children[2] = newChild
            else:
...
class TwoThreeFourTree:
    ...
    Een element heeft een key van het type KeyType.
    """
    def __init__(self):
        """
    ...
    def tableInsert(self, newItem):
        ...
        with items with different keys than the key of 'newItem'.
    ...
    def tableRetrieve(self, key):
        """
        Searches an item with 'key' as its key in the 2-3-4 tree and returns this item.
        Pre: 'key' is of the type KeyType
        Post: if there is an item with 'key' as key in the tree, it gets
        returned, else None is returned
        :param key: key of the item to be found.
        :return: item with 'key' as key and True if item exists, None and False if not.
        """
        return self.retrieve(self.root, key)

    def retrieve(self, node, key):
        """
        Searches an item with 'key' as its key in the 2-3-4 tree, starting at node, and returns this item.
        Pre: 'key' is of the type KeyType
        Post: if there is an item with 'key' as key in the tree, it gets
        returned, else None is returned
        ...
        for x in range(node.amount,0,-1):
            if node.items[x-1].key < key:
                return self.retrieve(node.children[x], key)
        return self.retrieve(node.children[0], key)
    ...
    def merge(self, node):
        ...
        if node.allSiblings1Nodes() and node.parent.amount == 1:
            ...
            if node.getSiblings()[0] == 1:
                sibling = node.getSiblings()[1]
                if node.getMaxkey() < node.getSiblings()[1].getMaxkey():
    ...
    def tableDelete(self, key):
        """
        Deletes the item with 'key' as search key from the 2-3-4 tree, if such item exists.
        Pre: 'key' is of the type KeyType.
        Post: there is no item with 'key' as search key in the 2-3-4 tree and the tree is a valid 2-3-4 tree.
        :param key: search key of the element that needs to be deleted.
        :return: True if deleting was successful, False if not.
        """
        if self.tableIsEmpty() or not self.tableRetrieve(key)[1]:
            return False
        searching = True
        current = self.root
        while searching:
