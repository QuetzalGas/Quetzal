...
class AdtTwoThreeTree:
    ...
    def compareAndPutInPlace(self, newItem):
        ...
        if self.nodeType == 2:                       # Als de knoop een 2-knoop is,
            self.nodeType = 3                           # zet nodeType al op 3, want deze zal zo meteen een 3-knoop worden
            if self.rootLeft.key > newItem.key:         # als de zoeksleutel van newItem kleiner is dan de zoeksleutel van rootLeft,
                self.rootRight = self.rootLeft          # ... Verplaats dan het item in rootLeft naar rootRight,
                self.rootLeft = newItem                 # ... zodat rootLeft kan ingenomen worden door newItem.
                return 0
        ...
        if self.nodeType == 3:                      # Als de knoop een 3-knoop is,
            self.nodeType = 4                           # zet nodeType al op 4, want deze zal zo meteen een 4-knoop worden
            if self.rootLeft.key > newItem.key:         # als de zoeksleutel van newItem kleiner is dan de zoeksleutels van de root,
                self.rootMid = self.rootLeft            # ... verplaats rootLeft naar rootMid,
                self.rootLeft = newItem                 # ... zodat rootLeft kan ingenomen worden door newItem
                self.midRight = self.midLeft            # ... verplaats midLeft naar midRight zodat er plaats is voor twee nieuwe kinderen langs links.
                return 2
            if self.rootRight.key > newItem.key:        # als de zoeksleutel kleiner is dan de grootste zoeksleutel van de root,
                self.rootMid = newItem                  # ... plaats dan newItem in het midden (aka rootMid)
                return 3
            if self.rootRight.key < newItem.key:        # als de zoeksleutel groter is dan de zoeksleutels in de root,
                ...
                self.rootRight = newItem                # ... zodat rootRIght ingenomen kan worden door newItem
                # self.midLeft = self.right               # ... verplaats het rechterkind naar midLeft, zodat er plaats is voor twee nieuwe kinder langs rechts
                return 4
    ...
    def split(self):
        ...
        if not self.isEmpty():              # Als de huidige node een interne knoop is, geen blad is:
            n1.left = self.left             # Plaats dan de twee linkerkinderen (left and midLeft) in n1
            n1.right = self.midLeft
            n2.left = self.midRight         # Plaats de twee rechterkinderen (right and midRight) in n2
            n2.right = self.right
            n1.left.parent = n1             # Zorg dat deze kinderen allemaal weten wat hun nieuwe ouder is (n1 of n2)
            n1.right.parent = n1
            n2.left.parent = n2
            n2.right.parent = n2

        result = pNode.compareAndPutInPlace(self.rootMid)       # Plaats in de pNode de middelste root van de node
        ...
        if result == -1:            # -1 : lege knoop (kinderen en root leeg)
            ...
            pNode.midLeft = n1          # pNode krijgt n1 en n2 als resp. linkermiddel en rechtermiddelkind
            pNode.midRight = n2
        else:                       # 4 : rootMid bevindt zich nu rechts in pNode, een 4-knoop
            pNode.midRight = n1         # pNode krijgt n1 en n2 als resp. rechtermiddel en rechterkind
            pNode.right = n2

        if wortelScenario:          # Als er een nieuwe knoop werd aangemaakt omdat de huidige node de wortel was, gn parent had,
                                    # dan moet de huidige knoop alles van pNode overnemen, omdat enkel zo men de nieuwe boom kan bereiken via de huidige knoop.
            self.rootMid = None
            ...
            self.midLeft = None                 # Als de wortel nog een middelste kind zou hebben, moet deze sowieso op None komen
            self.left.parent = self             # zorg zeker dat de kinderen weten welk hun nieuwe parent is
    ...
    def insert(self, newItem):
        ...
        if self.nodeType == 0:              # Als de node leeg is,
            self.rootLeft = newItem         # plaats dan in de root (standaard is dit rootLeft) het newItem,
        ...
        if self.nodeType == 3:              # Als de node een 3-knoop is,
            ...
            if self.isEmpty():                              # Als de 3-knoop geen kinderen heeft,
                self.compareAndPutInPlace(newItem)          # plaats dan newItem erbij,
                return self.split()                         # En voer de split-functie uit, want er is nu een knoop met 4 items.
            else:                                               # Als de 3-knoop wel kinderen heeft, zoek dan verder in
                if newItem.key < self.rootLeft.key:             # ...het linkerkind indien de zoeksleutel van newItem kleiner is dan de zoeksleutel van het kleinste item in de root
                    return self.left.insert(newItem)            # zo niet:
                elif newItem.key < self.rootRight.key:            # ...het middenkind indien de zoeksleutel van newItem kleiner is dan de zoeksleutel van het grootste item in de root
                    return self.midLeft.insert(newItem)         # zo niet:
                elif newItem.key > self.rootRight.key:            # ...het linkerkind als de zoeksleutel van newItem groter is dan de zoeksleutel van beide items in de root
                    return self.right.insert(newItem)
        if self.nodeType == 2:                              # Als de node een 2-knoop is...
            if self.isEmpty():                              # ... en de knoop is leeg,
                self.compareAndPutInPlace(newItem)          # plaats newItem in de knoop, en geef True terug.
                return True
            elif self.rootLeft.key > newItem.key:
                return self.left.insert(newItem)           # zoek dan verder in het rechterkind.
            else:
                return self.right.insert(newItem)
    ...
    def retrieve(self, key):
        ...
        if key == self.rootLeft.key:                # Als de key gelijk is aan de zoeksleutel van root Left, kan al meteen
            return (self.rootLeft, self, 1)         # de node, subtree en waarde 1 (wilt zeggen: zit in rootLeft) teruggeven worden
            ...
            if key == self.rootRight.key:           # en het is in de rootRight, dan kan deze worden teruggeven met waarde
                return (self.rootRight, self, 2)    # 2 (wilt zeggen: zit in rootRight)
            elif self.isEmpty():
                    return (None, None, 0)
            elif key < self.rootLeft.key:
                return self.left.retrieve(key)
    ...
    def retrieveItem(self, key):
        ...
        if self.rootLeft is None and self.rootRight is None:
            return (None, False)
        if key == self.rootLeft.key:                # Als de key gelijk is aan de zoeksleutel van root Left, kan al meteen
            return (self.rootLeft.item, True)         # de node, subtree en waarde 1 (wilt zeggen: zit in rootLeft) teruggeven worden
        elif self.nodeType == 2:
            if self.isEmpty():
                return (None, False)
            elif key < self.rootLeft.key:
            ...
            if key == self.rootRight.key:           # en het is in de rootRight, dan kan deze worden teruggeven met waarde
                return (self.rootRight.item, True)        # 2 (wilt zeggen: zit in rootRight)
            elif self.isEmpty():
                    return (None, False)
            elif key < self.rootLeft.key:
                return self.left.retrieveItem(key)
    ...
    def fix(self):
        ...
        # dat de node ook inderdaad als nodeType 2 heeft. (De node zal in het geval er al een fix is geweest nodeType = 0 hebben.)
        self.nodeType=2
        ...
        # lege nodes ontstonden. (de parent (in dit geval dus de wortel) en de node waar het fix algoritme op werd toegepast)
        if self.parent is None:                     # De wortel is leeg, zorg ervoor dat alles wordt overgenomen van de kinderen
            # Afhankelijk van welk kind niet leeg is, wordt alles overgenomen. De enigste opties zijn het rechter en linkerkind
            # want de node zou niet leeg zijn, moest het een middelste kind hebben (da zou het een 3-knoop zijn met 2 linker en rechterroot)
            if self.right is None:
                ...
                # het linkerkind al verdwenen zijn, voordat alles is overgenomen.
            ...
            self.setChildrenParent()    # Zorgt ervoor dat de kinderen weten dat ze een nieuwe ouder hebben
            return

        # Als de node een interne knoop is, dan kan het niet anders dan dat het maar 1 kind heeft (logica van het algoritme)
        # Om de implementatie te vergemakkelijken, zorg ik ervoor dat het altijd nog het linkerkind heeft (dat bespaart if-statements).
        if self.left is None:           # Als het dus een rechterkind is, wordt dit verplaatst naar het linkerkind
            self.left = self.right
            self.right = None

        # Volgende grote brok code, houdt zich bezig met de nodige verplaatsingen of merges in de verschillende gevallen
        ...
        if self.parent.left == self:
            ...
            # nu moet gewerkt worden met het middelste kind van de ouder, niet het rechterkind.
            ...
            # voor gezorgd, enkele lijnen terug)
                ...
                self.parent.left = None                                 # De node verwijderen als kind van de (lege) ouder
            ...
            # overnemen als midLeft en right kind. (die resp. het linker en rechterkind waren)
                ...
                self.parent.midLeft = None      # Ouder wordt een 2-knoop & alles is overgenomen van sibling, dus sibling mag weg
        ...
        # waar de node het linkerkind is.
            ...
            if self.parent.nodeType == 2 and self.parent.left.nodeType == 3:
                self.rootLeft = self.parent.rootLeft
                self.right = self.left                              # verplaatsen, voor het overnemen van het rechterkind van de sibling
                self.left = self.parent.left.right
                self.parent.left.right = self.parent.left.midLeft
                self.parent.left.midLeft = None                     # sibling is 2-knoop geworden, midleft is al verplaatst, dus zet op None
                self.parent.rootLeft = self.parent.left.rootRight
                self.parent.left.rootRight = None                   # sibling is 2-knoop geworden, rootRight is al overgenomen, dus zet op None
            ...
            # wel met het middelste kind van de ouder gewerkt, niet het linkerkind.
            elif self.parent.nodeType == 3 and self.parent.midLeft.nodeType == 3:
            ...
            # zijn rechterkind naar middelste kind, neemt het het linkerkind van de huidige node over als rechterkind.
                ...
                self.parent.right = None                            # de huidige node mag verwijderd worden (op None zetten bij ouder)
            ...
            # Ook wordt het rechterkind van de ouder niet op None gezet, maar juist vervangen door het middelste kind.
            elif self.parent.nodeType == 3 and self.parent.midLeft.nodeType == 2:
        ...
        # ouder te kijken voor de sibling. (men zou ook het linkerkind genomen kunnen hebben)
            ...
            # naar rootLeft verplaatsen, waarbij rootRight al meteen op None wordt gezet. De sibling wordt een 2-knoop.
            ...
            # De ouder wordt een 2-knoop en de sibling een 3-knoop. RootRight van de ouder moet verwijderd worden en het middelste kind ook.
        ...
        # Als de ouder leeg blijkt te zijn, moet het hele algoritme hier terug op uitgevoerd worden.
    ...
    def remove(self, key):
        ...
        # De root moet dus wijzen naar het volgende item in de linked list (python doet het nodige verwijderen).
        ...
        if self.parent is None and self.rootRight is None and self.isEmpty():   # Als het item het enigste item is in de
            self.rootLeft = None                                                # hele boom, moet dit item gewoon weg
            return True
        ...
        if subtree.isEmpty():
            if numb == 1 and subtree.nodeType == 2:
                subtree.rootLeft = None                 # node moet leeggemaakt worden (= item wordt verwijderd)
                return subtree.fix()
            ...
            # Belangrijk : nodeType wordt aangepast, want het wordt een 2-knoop
                ...
                subtree.nodeType = 2
            # Als het item zich in de rechterRoot bevind, moeten we rootRight op None zetten
            elif numb == 2:
                subtree.rootRight = None
                subtree.nodeType = 2
            ...
            if numb == 1 and subtree.nodeType == 3:                 # Als het item in rootLeft zit van een 3-knoop moet de inorder succesor
                inordSuc = subtree.midLeft.inorderSuccessor()       # in de middelste deelboom gezocht worden
            ...
            # rootLeft, want anders zou de hele essence van de inorder successor teniet gaan.
            ...
            if numb == 2:       # Het swappen van de inorder successor met het te-verwijderen-item als deze zich in rootRight bevind
                ...
                return inordSuc.remove(key)     # remove moet nu op de originele subtree van de inorder successor worden toegepast
            else:               # Het swappen als het te-verwijderen-item zich in rootLeft bevind (voor 2-knoop en 3-knoop)
    ...
    def printInorder(self):
        ...
        if self.nodeType == 2:          # Als het een 2-knoop is, die niet leeg is, dan moet printInorder van de 2 kinderen gedaan
                                        # worden volgens de logische volgorde van inorer traverse. Het item in de node moet ook geprint worden
            self.left.printInorder()
            print(self.rootLeft.key)
            self.right.printInorder()
        if self.nodeType == 3:          # Als het een 3-knoop is, die niet leeg is, dan moet printInorder van de 3 kinderen gedaan
                                        # worden volgens de logische volgorde van inorder traverse. De items in de node moeten ook geprint worden.
            self.left.printInorder()
            print(self.rootLeft.key)
    ...
    def createDotFile(self, filename):
        ...
        """
        # dotFile is de nieuwe file, (met bestandsnaam 'filename', en deze staat op 'w' want we willen de file aanpassen
        ...
        # Enkel als de boom niet volledig leeg is kan er met de addNodeToFile functie gewerkt worden:
        if self.rootLeft is not None:
            self.addNodeToFile(dotFile)     # de addNodeToFile functie zal de nodige nodes en edges toevoegen aan 'dotFile'
        dotFile.write("\n" + "}")
    ...
    def addNodeToFile(self, file):
        ...
        """
        leftKey = str(self.rootLeft.key)                 # leftKey is een string van de zoeksleutel van rootLeft, het liet me toe niet steeds str te doen
        nodeName = "node" + str(self.rootLeft.key)       # nodeName zal de naam van de node worden, steeds rootLeft, want deze is altijd aanwezig
        """
        ...
        if self.isEmpty():
            if self.nodeType == 2:
                ...
                file.write("\n" + nodeName + ' [label=" ' + leftKey + '| ' + str(self.rootRight.key) +'"];')
        ...
        # node werd bepaald door de key van de linkerRoot, dus deze kunnen we terugvinden als "node" + str(key parent)
        if self.parent is not None:
            if self.parent.rootLeft is None:
                print("error: ", self.rootLeft.key)
            nameParent = "node" + str(self.parent.rootLeft.key)
            file.write("\n" + nameParent + " -> " + nodeName + ";")     # edge van parent naar node, dus eerst parent dan node
